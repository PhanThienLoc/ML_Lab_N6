...
# The supplied Olist order-item stream falls from 7,248 items in August 2018
# to one item in September.  September is therefore a right-censored trailing
# period, not a normal observed sales month.  Keep August as the last usable
# demand/target month; feature rows then naturally end in July.
OLIST_USABLE_DEMAND_END_MONTH = pd.Timestamp("2018-08-01")
...
def build_category_month_dataset(
    ...
    *,
    usable_demand_end_month: str | pd.Timestamp | None = None,
    ...
    raw_item_monthly = (
        final.groupby("feature_month", as_index=False)
        .agg(order_item_demand=("product_id", "size"), order_count=("order_id", "nunique"))
        .set_index("feature_month")
    )
    valid_orders["feature_month"] = valid_orders["order_purchase_timestamp"].dt.to_period("M").dt.to_timestamp()
    raw_order_monthly = valid_orders.groupby("feature_month").size()

    trailing_period_policy: dict[str, Any] = {
        "applied": usable_demand_end_month is not None,
        "reason": None,
        "usable_demand_end_month": None,
        "excluded_months": [],
        "excluded_order_item_rows": 0,
        "boundary_month_observations": [],
    }
    if usable_demand_end_month is not None:
        cutoff = pd.Timestamp(usable_demand_end_month).to_period("M").to_timestamp()
        observed_end = max(final["feature_month"].max(), valid_orders["feature_month"].max())
        context_start = cutoff - pd.offsets.MonthBegin(2)
        context_months = pd.date_range(context_start, observed_end, freq="MS")
        trailing_period_policy = {
            "applied": True,
            "reason": (
                "Exclude right-censored trailing Olist demand periods after the configured usable end month; "
                "they are not treated as complete sales targets."
            ),
            "usable_demand_end_month": cutoff.strftime("%Y-%m"),
            "excluded_months": [
                month.strftime("%Y-%m")
                for month in raw_item_monthly.index
                if month > cutoff
            ],
            "excluded_order_item_rows": int((final["feature_month"] > cutoff).sum()),
            "boundary_month_observations": [
                {
                    "month": month.strftime("%Y-%m"),
                    "order_item_demand": int(raw_item_monthly.loc[month, "order_item_demand"])
                    if month in raw_item_monthly.index
                    else 0,
                    "order_count": int(raw_order_monthly.get(month, 0)),
                }
                for month in context_months
            ],
        }
        final = final.loc[final["feature_month"] <= cutoff].copy()
        if final.empty:
            raise ValueError("The usable demand end month removes every joined order-item row.")
    ...
    evidence: dict[str, Any] = {
        ...
        "trailing_period_policy": trailing_period_policy,
